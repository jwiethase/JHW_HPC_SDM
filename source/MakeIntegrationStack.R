...
#' @export
#' @import INLA
MakeIntegrationStack <- 
  function(mesh, data, area, tag='mesh',
  ...
  #create inla stack
  stk.ip <- inla.stack(data=list(resp=NA, e=area),  # 0 for count, NA for incidental, e is area of mesh polygon
                       A=list(1,projmat.ip), tag=tag,
                       effects=list(NearestCovs@data, list(i=1:mesh$n)))   # mesh$n is number of polygons, index is added to data set

  # We only have incidental data, so change function slightly
  # stk.ip <- inla.stack(data=list(resp=NA, e=area),  # 0 for count, NA for incidental, e is area of mesh polygon
  #                      A=list(1,projmat.ip), tag=tag,                     
