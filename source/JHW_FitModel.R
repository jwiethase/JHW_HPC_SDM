...
FitModel <- function (..., formula = NULL, CovNames = NULL, mesh, spat.ind = "i", 
                       ...
                       waic = FALSE, dic = FALSE, cpo = NULL, nthreads = NULL, verbose = FALSE) 
{
  stck <- inla.stack(...)
  ...
  mod <- inla(Formula, family = "binomial", control.family = list(link = "cloglog"), data = inla.stack.data(stck), 
              ...
              control.compute = list(waic = waic, dic = dic, cpo = cpo))
  if (predictions) {
